...
annotated AS (
    ...
    -- LEFT so a database with no alignments still returns every other
    -- dimension; the alignment dimensions come back empty rather than the
    -- whole query failing.
    LEFT JOIN variant_alignments AS a ON a.variant_id = s.variant_id
