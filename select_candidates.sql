-- Rank designs by binding free energy, most positive first.
--
-- Sort direction is DESC on purpose. delta_g_binding is negative for a design
-- that binds, so the most positive values are the designs whose binding is most
-- disrupted. On a mutation and truncation scan that is usually the question:
-- which changes cost the most binding. Flipping to ASC gives the opposite
-- ranking, the strongest binders, so do not "fix" this to ASC without meaning
-- it. The direction is not parameterised because a bind parameter cannot appear
-- in ORDER BY; sql/select_strongest_binders.sql is the ASC counterpart.
--
...
ranked AS (
    SELECT
        ...
        ROW_NUMBER() OVER (
            ...
            ORDER BY delta_g_binding_kcal DESC, variant_id ASC
        ) AS rank_in_class
    FROM filtered
)
