...
from .engines import TargetAligner
from .models import Target, Variant, VariantAlignment
...
def align_variants(session: Session, target: Target, batch_size: int = 5000) -> int:
    """Fill in variant_alignments for every design that has no row yet.

    Idempotent, and cheap enough to just call before every run: aligning the
    full 54,660 design scan against a 76 nt target takes under a second because
    the window comparison is vectorised.
    """
    aligner = TargetAligner(target.sequence)

    already = {
        row[0]
        for row in session.execute(
            select(VariantAlignment.variant_id).where(
                VariantAlignment.target_id == target.target_id
            )
        ).all()
    }
    ...
    pending = []
    written = 0
    rows = session.execute(select(Variant.variant_id, Variant.sequence)).all()

    for variant_id, sequence in rows:
        if variant_id in already:
            continue
        ...
        pending.append(
            VariantAlignment(
                ...
                n_complementary=alignment.n_complementary,
                n_mismatch=alignment.n_mismatch,
